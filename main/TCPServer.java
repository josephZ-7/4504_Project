...
public class TCPServer {
	...
	private static int[][] binaryTreeMultiply(List<int[][]> matrices, int threadCount) {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		// Thread count local variable determines how many cores the multi-threaded job uses.
		int threadCount = 31;
		...
		// Receive matrices from router
		List<int[][]> fromRouter = (List<int[][]>) in.readObject();
		System.out.println("Server: Matrices received from router.");

		// First calculation with 1 thread
		System.out.println("Server: Starting single-threaded multiplication...");
		long singleThreadStart = System.nanoTime();
		int[][] resultSingleThread = binaryTreeMultiply(fromRouter, 1);
		long singleThreadEnd = System.nanoTime();
		double singleThreadTime = (singleThreadEnd - singleThreadStart) / 1_000_000.0;
		System.out.println("Server: Single-threaded multiplication complete in " + singleThreadTime + " ms");

		// Send first result
		out.writeObject(resultSingleThread);
		System.out.println("Server: Single-threaded result sent.");

		// Reconnect for second calculation with n (threadCount) threads
		try {
			socket = new Socket(hostName, portNumber);
			out = new ObjectOutputStream(socket.getOutputStream());
			in = new ObjectInputStream(socket.getInputStream());
		...
		// Receive matrices again
		fromRouter = (List<int[][]>) in.readObject();

		// Second calculation with n (threadCount) threads
		System.out.println("Server: Starting multi-threaded multiplication...");
		long multiThreadStart = System.nanoTime();
		int[][] resultMultiThread = binaryTreeMultiply(fromRouter, threadCount);
		long multiThreadEnd = System.nanoTime();
		double multiThreadTime = (multiThreadEnd - multiThreadStart) / 1_000_000.0;
		System.out.println("Server: Multi-threaded multiplication complete in " + multiThreadTime + " ms");

		// Send second result
		out.writeObject(resultMultiThread);
		System.out.println("Server: Multi-threaded result sent.");
		...
		System.out.println("Speedup: " + String.format("%.2fx", singleThreadTime / multiThreadTime));
